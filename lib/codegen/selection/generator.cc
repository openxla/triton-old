...
Value* geper::operator()(Type *ty, Value *ptr, Value* off, const std::string& name){
  ...
  if(ConstantInt* cst = dyn_cast<ConstantInt>(bin->getOperand(1))){
    Value *gep = (*builder_)->CreateGEP(ty, ptr, bin->getOperand(0));
    return (*builder_)->CreateGEP(ty, gep, bin->getOperand(1));
  }
  // default
...
//  builder_->SetInsertPoint(tid_0_bb);
  ...
  tgt_->add_memfence(module, *builder_);
  add_barrier();
  vals_[cas][{}] = load(cvt(cas->get_type()->get_scalar_ty()), atom_ptr);
  add_barrier();
}
...
void generator::visit_atomic_rmw_inst(ir::atomic_rmw_inst *atom) {
  ...
  for(int i = 0; i < idxs_.at(val).size(); i += vec){
    ...
    if(atom->get_type()->is_block_ty())
      ...
      store(old, atom_ptr);
      add_barrier();
      vals_[atom][idx] = load(ptr_ty(old->getType(), 3), atom_ptr);
      add_barrier();
    }
  }
}
...
void generator::visit_mma884(ir::dot_inst* C, ir::value *A, ir::value *B, ir::value *D, unsigned NK) {
  ...
  Type *a_ty = cvt(layout_a->get_type());
  Type *b_ty = cvt(layout_b->get_type());
  for(int i = 0; i < num_ptr_a; i++)
    ptr_a[i] = gep(a_ty, shmems_[A], off_a[i]);
  for(int i = 0; i < num_ptr_b; i++)
    ptr_b[i] = gep(b_ty, shmems_[B], off_b[i]);
  ...
  auto load_a = [&](int m, int K, int inc, bool is_prefetch) {
    ...
    if(K==0 && is_prefetch){
      if(inc == 0)
        ptra = gep(a_ty, shared_pre_ptr_[layout_a], off_a[offidx]);
      else
        ptra = gep(a_ty, shared_next_ptr_[layout_a], off_a[offidx]);
    }
    else
      ptra = ptr_a[offidx];
    int step_am = is_a_row ? m : m / (num_ptr_a)*(num_ptr_a);
    int step_ak = is_a_row ? K / (num_ptr_a*vec_a)*(num_ptr_a*vec_a) : K;
    Value* pa =  gep(a_ty, ptra, i32(step_am*stride_rep_m*stride_am + step_ak*stride_ak));
    Value* ha = load(vec_ty(i32_ty, vec_a/2), pa);
  ...
  auto load_b = [&](int n, int K, int inc, bool is_prefetch) {
    ...
    if(K==0 && is_prefetch){
      if(inc == 0)
        ptrb = gep(b_ty, shared_pre_ptr_[layout_b], off_b[offidx]);
      else
        ptrb = gep(b_ty, shared_next_ptr_[layout_b], off_b[offidx]);
    } else
      ptrb = ptr_b[offidx];

    int stepbn = is_b_row ? n / (num_ptr_b)*(num_ptr_b) : n;
    int stepbk = is_b_row ? K : K / (num_ptr_b*vec_b)*(num_ptr_b*vec_b);
    Value* pb =   gep(b_ty, ptrb, i32(stepbn*stride_rep_n*stride_bn + stepbk*stride_bk));
    Value* hb =   load(vec_ty(i32_ty, vec_b/2), pb);
    // record lds that needs to be moved
    if (K == 0 && inc == 1 && is_prefetch)
...
public:
  ...
  load_x4(int mat0, int mat1, int inc, bool is_prefetch, ir::phi_node *pn,
    ...
    if (can_use_ldmatrix_) {
      ...
      if (k_order_ == 1) {
        elem0 = load(smem_pointee_ty, gep(smem_pointee_ty, ptr,  i32(s_offset_elem)));
        elem1 = load(smem_pointee_ty, gep(smem_pointee_ty, ptr2, i32(s_offset_elem)));
        elem2 = load(smem_pointee_ty, gep(smem_pointee_ty, ptr,  i32(s_offset_elem + s_offset_arr_elem)));
        elem3 = load(smem_pointee_ty, gep(smem_pointee_ty, ptr2, i32(s_offset_elem + s_offset_arr_elem)));

      } else { // for b (k first)
        elem0 = load(smem_pointee_ty, gep(smem_pointee_ty, ptr,  i32(s_offset_elem)));
        elem2 = load(smem_pointee_ty, gep(smem_pointee_ty, ptr2, i32(s_offset_elem)));
        elem1 = load(smem_pointee_ty, gep(smem_pointee_ty, ptr,  i32(s_offset_elem + s_offset_arr_elem)));
        elem3 = load(smem_pointee_ty, gep(smem_pointee_ty, ptr2, i32(s_offset_elem + s_offset_arr_elem)));
      }
      if (k == 0 && inc == 1 && is_prefetch) {
      ...
      Value *i8_elems[4*4];
      if (k_order_ == 1) { //
        i8_elems[0*4 + 0] = load(i8_ty, gep(i8_ty, ptr00, i32(s_offset_elem)));
        i8_elems[0*4 + 1] = load(i8_ty, gep(i8_ty, ptr01, i32(s_offset_elem)));
        i8_elems[0*4 + 2] = load(i8_ty, gep(i8_ty, ptr02, i32(s_offset_elem)));
        i8_elems[0*4 + 3] = load(i8_ty, gep(i8_ty, ptr03, i32(s_offset_elem)));

        assert(i8_elems[0*4 + 0]->getType()->isIntegerTy(8));

        i8_elems[1*4 + 0] = load(i8_ty, gep(i8_ty, ptr10, i32(s_offset_elem)));
        i8_elems[1*4 + 1] = load(i8_ty, gep(i8_ty, ptr11, i32(s_offset_elem)));
        i8_elems[1*4 + 2] = load(i8_ty, gep(i8_ty, ptr12, i32(s_offset_elem)));
        i8_elems[1*4 + 3] = load(i8_ty, gep(i8_ty, ptr13, i32(s_offset_elem)));

        i8_elems[2*4 + 0] = load(i8_ty, gep(i8_ty, ptr00, i32(s_offset_elem + s_offset_arr_elem)));
        i8_elems[2*4 + 1] = load(i8_ty, gep(i8_ty, ptr01, i32(s_offset_elem + s_offset_arr_elem)));
        i8_elems[2*4 + 2] = load(i8_ty, gep(i8_ty, ptr02, i32(s_offset_elem + s_offset_arr_elem)));
        i8_elems[2*4 + 3] = load(i8_ty, gep(i8_ty, ptr03, i32(s_offset_elem + s_offset_arr_elem)));

        i8_elems[3*4 + 0] = load(i8_ty, gep(i8_ty, ptr10, i32(s_offset_elem + s_offset_arr_elem)));
        i8_elems[3*4 + 1] = load(i8_ty, gep(i8_ty, ptr11, i32(s_offset_elem + s_offset_arr_elem)));
        i8_elems[3*4 + 2] = load(i8_ty, gep(i8_ty, ptr12, i32(s_offset_elem + s_offset_arr_elem)));
        i8_elems[3*4 + 3] = load(i8_ty, gep(i8_ty, ptr13, i32(s_offset_elem + s_offset_arr_elem)));
        ...
        i8_elems[2*4 + 0] = load(i8_ty, gep(i8_ty, ptr10, i32(s_offset_elem)));
        i8_elems[2*4 + 1] = load(i8_ty, gep(i8_ty, ptr11, i32(s_offset_elem)));
        i8_elems[2*4 + 2] = load(i8_ty, gep(i8_ty, ptr12, i32(s_offset_elem)));
        i8_elems[2*4 + 3] = load(i8_ty, gep(i8_ty, ptr13, i32(s_offset_elem)));

        i8_elems[1*4 + 0] = load(i8_ty, gep(i8_ty, ptr00, i32(s_offset_elem + s_offset_arr_elem)));
        i8_elems[1*4 + 1] = load(i8_ty, gep(i8_ty, ptr01, i32(s_offset_elem + s_offset_arr_elem)));
        i8_elems[1*4 + 2] = load(i8_ty, gep(i8_ty, ptr02, i32(s_offset_elem + s_offset_arr_elem)));
        i8_elems[1*4 + 3] = load(i8_ty, gep(i8_ty, ptr03, i32(s_offset_elem + s_offset_arr_elem)));
  ...
  for(unsigned k = 0; k < NK; k++){
    ...
    for(unsigned jj = 0; jj < layout_c->nts(ord[0]); jj++){
      ...
      if(has.find({m + mm, k}) == has.end()){
        ...
        Value* va = load(f_mul_add->getFunctionType()->getParamType(0), pa);
        has[{m + mm, k}] = va;
      }
      if(hbs.find({n + nn, k}) == hbs.end()){
        Value* pb = gep(ty_b, ptrs_b[0], i32((n + nn)*stride_b_n + k*stride_b_k));
        Value* vb = load(f_mul_add->getFunctionType()->getParamType(1), pb);
        hbs[{n + nn, k}] = vb;
      }
...
void generator::visit_reducend_inst(ir::reduce_inst* x, acc_fn_t do_acc, Value *neutral) {
  ...
  for(auto& x: accs) {
    ...
    for(size_t i = shape[axis]/2; i > 0; i >>= 1){
      ...
      // update accumulator
      do_acc(
          acc, [&]() -> Value * { return load(data_ty, read_ptr); },
          [&]() -> Value * { return load(index_ty, index_read_ptr); }, false);
      add_barrier();
      store(acc.first, write_ptr);
      if (with_index) {
        store(acc.second, index_write_ptr);
  ...
  for(indices_t idx: idxs_.at(x)){
    ...
    vals_[x][idx] = load(read_ty, read_ptr);
  };
}
...
void generator::visit_layout_convert(ir::value *out, ir::value *in){
  ...
  for(int j = 0; j < n_reps[1]; j++){
    ...
    for(int jj = 0; jj < max_jj; jj+=out_vec){
      ...
      Value* vals = load(vec_ty(ty, out_vec), ptr);
